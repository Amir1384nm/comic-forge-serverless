"""RunPod cached-model worker for H3 and future validated WAN workflows."""
...
def handler(job: dict) -> dict:
    ...
    if operation != "workflow":
        raise ValueError("Unsupported video worker operation")
    ...
    artifacts, inference = RUNNER.run_workflow(
        request.get("workflow") or {}, timeout=int(request.get("timeout") or 1800)
    )
    videos = [item for item in artifacts if item["mime_type"].startswith("video/")]
    if not videos:
        raise RuntimeError("Video workflow returned no video")
    return {
        "schema": 1,
        "artifacts": videos,
        "metrics": {
